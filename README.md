...
League of Legends is without a doubt one of the world's most popular mulitplayer online battle arena (MOBA) game out there. A basic outline of how the game works includes two teams of five, where they face against each other to see who destroys the other home-base first. Each match has a map comprising of three lanes that leads to each team's base: top, middle, bottom. Within these lanes is this concept of "towers". Towers act as defensive structures that damage enemy players within range, protecting the base. Destroying towers not only provides an economic advantage but also strategic map control.

This leads us into the central question we'll be investigating: **"How well do the early game metrics contribute to teams getting the first tower in a match?"** This is where we will be doing data analysis to unravel the correlation between the early game variables and whether or not that actually contributed to a team getting the first tower within their match. Later throughout this project, we will be using these early game variables and more to see whether or not it is able to predict the amount of towers a team gets in a match. The models we're going to create down the line will influence strategic-decision making, prioritization of certain objectives, team compositions, ultimately improving overall performance.

### Dataset Introduction
In our project, we are going to be working with Oracle Elixir's League of Legends professional match's history throughout 2022. This datatset consists of 150,588 rows and 161 columns. It contains a wealth of in-game statistics, including key gameplay metrics that help us answer our research question and support our predictive analysis.

Below are some of the key columns in our dataset:
...
The histogram shows us that most teams have relatively few kills near the start of the game, with the distribution being heavily skewed to the right. Majority of teams will have between 0-5 kills with a small fraction having more than that, and its unimodal with peak at around 1 kill.
...
The distribution here shows us that it is approximately normal. This tells us that within the early game, majority of teams will get around 320 minion kills. This further suggests that early-game laning performance is relatively consistent across teams.

### Bivariate Analysis

Below is a scatterplot demonstrating the relationship between kills at 10 minutes and CS at 10 minutes:
<iframe
...
In our dataset, I believe that columns that are NMAR (Not Missing at Random) would be doublekills', 'triplekills', 'quadrakills', 'pentakills'. Reason why I think this is the case is because it depends on whether or not the actual player gets those type of kills consecutively. For example, support players typically do not reach quadruple or penta kills. Thus, their missing values may indicate that these players inherently cannot achieve such high kill streaks. An extra column we might add to make this MAR, is a 'had_killstreak' column, where it's a binary indicator of whether or not the player had 2 or more kills in a row in a match. 1 indicating that they did, 0 if not. 
...
<iframe
  ...
  style="display:block; margin-bottom:0;"
></iframe>
Here, the observed test statistic (TVD) is 0.0, with a p-value of 1. This means that we accept the null and it indicates that the missingness of 'firsttower' does not depend on 'side' column. 

## Hypothesis Testing

In this hypothesis test, we're going to see whether or not there is a significant difference in the distributions of 'csat10' between teams that did get the first tower and the teams that didn't. Tying back to the bivariate graph we made earlier, I'm going to answer "are those two distributions that different?".
...
## Fairness Analysis
In this section, we're going to test if our model performs fairly across different groups. We're going to investigate the question of: "does my model perform worse for teams that has 500 minion kills or higher than it does for teams that got less than 500 minion kills?”. 

We're going to be performing a permutation test where our group x are the teams that got more than or equal to 500 minion kills in their respective match, and group y being the teams that got less than 500 minion kills.
...
**Alternative Hypothesis**: The r^2 score of teams that have less than 500 minion kills is **NOT** the same as the teams who have greater than or equal to 500 minion kills. Meaning the model is unfair.
